...
(** Intro patterns can be nested to destruct values from composed
datatypes.
Consider the following example where a value of the [color] type is destructed
using the pattern [[ | |[]]]. This intro pattern indicates that [c] gets
destructed into three cases (corresponding to [black], [white] and [primary p])
where the argument of the last case (corresponding to [primary]'s argument [p])
again gets destructed (as indicated by the subpattern [[]]).
This leaves the proof with 5 subcases, corrsponding to [black], [white],
[primary red], [primary green], and [primary blue]. *)

Example monochrome_not_red : forall c,
   monochrome(c) = true -> isred(c) = false.
   Proof.
    intros c H.
    destruct c as [ | | [] ].
    - simpl. reflexivity.
